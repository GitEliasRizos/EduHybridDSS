...
class ProblemValidator:
    """Validator for problem configurations"""
    ...
    def validate_problem_config(config: Dict[str, Any]) -> List[str]:
        ...
        if 'objectives' in config:
            errors.extend(ProblemValidator._validate_objectives(config['objectives'], config.get('variables', [])))
            
        # Validate constraints
        if 'constraints' in config:
            errors.extend(ProblemValidator._validate_constraints(config['constraints'], config.get('variables', [])))
